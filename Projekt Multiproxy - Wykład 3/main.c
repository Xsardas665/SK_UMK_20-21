...
#define BACKLOG 10
/**
 * Definiuje Długość Buffera 
 */
#define MAXBUF 65535
/**
 * Definiuje Maxymalną liczbę połączeń
 */
...
#endif
/**
 * Tworze strukturę proxy która przechowuje porty i adres proxy.
 */
...
/**
 * Tworzę strukturę connection która ma za zadanie utrzymywać proxy.
 */
...
/**
 * Definiuje zmienne globalne uzywane w programie
 */
char error[256];
...
int main(int argc, char **argv){
    ...
    /**
     * Deklaruję ilość miejsc w tablicy zawierającej moje struktury Proxy.
     */
    proxy p[argc]; 
    ...
    proxies = p;
    /**
     * Wyliczam ilość proxy które zostaną stworzone.
     */
    proxiesCount = ParseArgs(&argc, argv, proxies);

    /**
     * Sprawdzam czy nie przekroczono maxymalnej ilości proxy
     */
    ...
    /**
     * Otwieram decryptor plików epoll z flagą 0
     */
    ...
    /**
     * Dla każdego proxy z tablicy tworzę gniazdo i ustawiam niezbędne parametry,
     */
    ...
    while (1) {
        ...
        /**
         * Oznaczam poszczególne proxy jako gotowe
         */
        for (int i = 0; i < fds; i++) {
        ...
        /**
         * Dla wszystkich gotowych proxy otwieram połączenia
         */
        for (int i = 0; i < proxiesCount; i++) {
...
/**
 * Metoda tworząca gniazda klienckie
 */
int CreateClientSocket(char *address, char *port) {
...
/**
 * Metoda tworząca gniazda serwerowe
 */
int CreateServerSocket(char *address, char *port) {
    int sockfd, gai_error;
    struct addrinfo hints, *servinfo, *p;
...
/**
 * Metoda służąca do rozpoznania i obsługi argumentów
 */
int ParseArgs(int *argc, char **argv, proxy *proxies) {
    if (*argc == 1) {
        printf("Usage:\n");
...
/**
 * metoda wyszukująca odpowiednie proxy
 */
proxy *FindProxy(int fd) {
...
/**
 * Metoda służąca do zdobycia Adresu
 */
...
/**
 * Metoda tworząca połączenie proxy
 */
void AddConnection(int serverfd) {
...
/**
 * Metoda służąca do wymiany danych po proxy
 */
void SendRecv(int from, int to, int *readReady, int *writeReady, buff *b, connection *c) {
    int k, n;
...
/**
 * Ustawianie trybu Braku Blokowania
 */
void SetNonBlocking(int fd) {
    int flags;
...
/**
 * Metoda do oznaczania poszczególnych połączeń jako gotowe
 */
void SetReady(int fd) {

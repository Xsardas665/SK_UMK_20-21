...
int main(int argc, char** argv)
{
	...
	else if (strcmp(argv[1], "-p") == 0)
	{
		...
		else
		{
			...
			signal(SIGTERM, detach); // poprawne zamknięcie
			signal(SIGINT, detach); // ctrl+c
...
void parent(pid_t pid)
{
	printf("Format: syscall_name(rdi, rsi, rdx, r10, r8, r9) = syscall_return_value\n");
	...
	while (1)
	{
		...
		if (WIFEXITED(status))
		{
			if (resp == 1)
				...
				printf("%s(%llu, %llu, %llu, %llu, %llu, %llu) = ?\n", syscalltostring(regs.orig_rax), regs.rdi, regs.rsi, regs.rdx, regs.r10, regs.r8, regs.r9);
		...
		if (ptrace(PTRACE_GETREGS, pid, 0, &regs) == -1)
			printerror("PTRACE_GETREGS");

		if (resp == 0)
			printf("%s(%llu, %llu, %llu, %llu, %llu, %llu)", syscalltostring(regs.orig_rax), regs.rdi, regs.rsi, regs.rdx, regs.r10, regs.r8, regs.r9);
...
char* syscalltostring(long id)
{
	//ids from https://blog.rchapman.org/posts/Linux_System_Call_Table_for_x86_64/
	switch (id)
	{
	case 0:
		return "read";
